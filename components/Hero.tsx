export function Hero() {
  return (
    <section className="min-h-[100dvh] min-h-[100svh] bg-neutral-950 relative overflow-hidden flex flex-col">
      ...
      <div
        className="absolute inset-0 hidden sm:flex justify-between px-6 md:px-16 lg:px-24 pointer-events-none"
        aria-hidden
      >
      ...
      <div className="container mx-auto px-3 sm:px-6 pt-24 sm:pt-28 md:pt-36 pb-10 sm:pb-16 md:pb-24 relative z-10 flex flex-col flex-1 justify-center min-w-0">
        <div className="flex flex-col items-center text-center w-full max-w-5xl mx-auto min-w-0">
          <div className="mb-6 sm:mb-8 md:mb-10 w-full flex justify-center px-1">
            <span className="inline-flex flex-wrap items-center justify-center gap-2 bg-neutral-900/80 backdrop-blur-xl border border-neutral-700/50 text-neutral-400 px-3 py-2 sm:px-5 sm:py-2.5 rounded-full text-[10px] sm:text-xs md:text-sm font-medium tracking-wide sm:tracking-wider uppercase text-balance max-w-full leading-snug">
              <span className="w-2 h-2 bg-amber-500 rounded-full animate-pulse shrink-0" aria-hidden />
              Детективное агентство для сложных задач
            </span>
          </div>

          <h1 className="mb-5 sm:mb-6 md:mb-8 font-bold tracking-tight text-balance w-full px-1">
            <span className="block text-[clamp(2rem,11vw,2.75rem)] sm:text-5xl md:text-7xl lg:text-8xl text-gold-shimmer leading-[1.05]">
              PRIORITY
            </span>
            <span className="block text-[clamp(2rem,11vw,2.75rem)] sm:text-5xl md:text-7xl lg:text-8xl text-neutral-100 mt-1 sm:mt-2 leading-[1.05]">
              AGENCY
            </span>
          </h1>

          <p className="text-base sm:text-xl md:text-2xl lg:text-3xl text-neutral-300 mb-3 sm:mb-4 md:mb-6 max-w-3xl mx-auto leading-snug sm:leading-relaxed font-light tracking-wide text-balance px-2">
            Находим то, что другие упускают
          </p>

          <p className="text-sm sm:text-base md:text-lg text-neutral-500 mb-8 sm:mb-10 md:mb-14 max-w-2xl mx-auto leading-relaxed text-pretty px-2">
          ...
          <div className="mt-8 sm:mt-16 md:mt-24 animate-bounce max-sm:scale-90">
            <span className="text-neutral-600 text-[10px] sm:text-xs tracking-[0.2em] sm:tracking-[0.3em] uppercase mb-2 sm:mb-3 block">
              Листайте ниже
            </span>
        ...
        <div className="container mx-auto px-3 sm:px-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-neutral-800/50">
            {[
              ...
              <div
                key={index}
                className="min-w-0 bg-neutral-950/95 py-4 sm:py-6 md:py-8 px-2 sm:px-4 md:px-8 text-center group hover:bg-neutral-900/80 transition-colors duration-300"
              >
                <h3 className="text-xl sm:text-3xl md:text-4xl font-bold text-amber-500/90 mb-0.5 sm:mb-1 tabular-nums">
                  {stat.number}
                </h3>
                <p className="text-neutral-300 font-medium text-[11px] leading-tight sm:text-sm md:text-base mb-0.5 text-balance">
                  {stat.label}
                </p>
                <p className="text-neutral-600 text-[10px] sm:text-xs hidden sm:block leading-snug">{stat.sublabel}</p>
              </div>
            ))}
          </div>
        </div>
